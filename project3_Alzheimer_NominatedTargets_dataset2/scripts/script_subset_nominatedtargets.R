...
#set working dirtory
getwd()
setwd("C:/R_project/project3_Alzheimer_NominatedTargets")
...
gene_order <- rownames(scaled_data)[hc$order]

DoHeatmap(target_seurat,
          features = gene_order,
          slot = "scale.data",
          group.by = "condition",
          raster = TRUE)
# Get group information (replace 'groups' with your metadata column name)
group_info <- target_seurat@meta.data$condition # should be 'hc' and 'patient'

# Create annotation data frame for columns
col_anno <- data.frame(Group = group_info)
rownames(col_anno) <- colnames(target_seurat)
...
# Create heatmap with group splitting
pheatmap(scaled_data,
         show_rownames = FALSE,    # Hide row names for clarity
         show_colnames = FALSE,    # Hide column names
         cluster_rows = TRUE,      # Cluster rows
         cluster_cols = FALSE,      # Cluster columns
         annotation_col = col_anno, # Add group annotation
         scale = "none",          # Data is already scaled
         color = colorRampPalette(c("gray", "white", "red"))(100),
         gaps_col = cumsum(table(group_info)[unique(group_info)])) # Add gaps between groups



target_seurat <- RunPCA(merged_seurat, features = VariableFeatures(object = merged_seurat))
